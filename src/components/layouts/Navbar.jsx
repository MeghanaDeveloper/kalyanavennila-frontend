      ...
      <nav className="padding-lr bg-primary p-4 shadow-md fixed top-0 left-0 w-full text-white z-50">
          ...
          <div className="hidden md:flex items-center space-x-3.5 font-medium">
            {!isAuthenticated ? (
              ...
              <>
                ...
                <Link to="#" className="hover:text-gray-300">Find Your Match</Link>
                <Link to="/about" className="hover:text-gray-300">About Us</Link>
                <Link to="/create-profile" className="hover:text-gray-300">Create Profile</Link>
                ...
                <div  className="relative">
                  ...
                  {isOpen === "dropdown" && (
                    <div className="absolute right-0 mt-2 w-40 bg-white text-black shadow-lg rounded-lg">
                      <Link to="/profile" className="cursor-pointer block px-4 py-2 hover:bg-gray-100">View Profile</Link>
                      <p 
                        onClick={() => {
                          setIsOpen(null); 
                          dispatch(logout());
        ...
        {isOpen === "mobile" && (
          <div className="md:hidden flex flex-col bg-[#F7641E] p-4 space-y-3">
            {!isAuthenticated ? (
              <>
                <p onClick={() => [setIsSignUpOpen(true), setStep(1), setIsOpen(null)]} className="rounded-lg cursor-pointer shadow-2xl px-3 py-1.5 transition">Sign Up</p>
                <p onClick={() => [setIsLoginOpen(true), setStep(8), setIsOpen(null)]} className="hover:text-gray-300 rounded-lg cursor-pointer px-3 py-1.5 transition">Login</p>
              </>
            ) : (
              <>
                <Link to="/home" className="hover:text-gray-300" onClick={() => setIsOpen(null)}>Home</Link>
                <Link to="#" className="hover:text-gray-300" onClick={() => setIsOpen(null)}>Find Your Match</Link>
                <Link to="/about" className="hover:text-gray-300" onClick={() => setIsOpen(null)}>About Us</Link>
                <Link to="/create-profile" className="hover:text-gray-300" onClick={() => setIsOpen(null)}>Create Profile</Link>
                <Link to="/profile" className="hover:text-gray-300" onClick={() => setIsOpen(null)}>View Profile</Link>
                <p onClick={() => { dispatch(logout()); setIsOpen(null); }} className="hover:text-gray-300 rounded-lg cursor-pointer transition">
                  Logout
                </p>
              </>
            )}
